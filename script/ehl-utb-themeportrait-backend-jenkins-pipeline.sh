...
if [ "$#" -ge 3 ]; then
  ...
  *)
  ...
  remote_package_dir="${jenkinsWorkspace}/ehl-utb-themeportrait-microservice/target"
  app_package_dir="/root/apps/${branch}"
  app_backup_dir="/home/ehl-utb-themeportrait-web.bak/${branch}"
  ...
  scp -i ~/.ssh/id_rsa_172.38.90.57 root@172.38.90.57:"${remote_package_dir}"/ehl-utb-themeportrait-web.war "${app_package_dir}/"
  ...
  echo ">>> remove previous war and deployed files in '${tomcat_root_dir}/webapps/ehl-utb-themeportrait-web' done!"

  # deploy backend package
  cp "${app_package_dir}"/ehl-utb-themeportrait-web.war ${tomcat_root_dir}/webapps/
  ...
  sed -i "/axiosUrl/{s/[0-9]\{4\}/${port}/g}" ${tomcat_root_dir}/webapps/ehl-utb-themeportrait-web/WEB-INF/classes/static/static/config/js/url.js
  sed -i "/90.91:/{s/[0-9]\{4\}/${port}/g}" ${tomcat_root_dir}/webapps/ehl-utb-themeportrait-web/WEB-INF/classes/static/static/config/js/url.js
  echo ">>> recover front-end in '${tomcat_root_dir}/webapps/ehl-utb-themeportrait-web/WEB-INF/classes/' done!"
